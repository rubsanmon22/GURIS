...
module mod_config
  ...
  type :: config_t
     ...
     integer :: trajectory_stride = 1
     integer :: trajectory_npart = 0
     integer:: trajectory_nids = 0
     integer, dimension(100) :: trajectory_ids = 0
...
contains
  ...
  subroutine read_config(filename, cfg)
    ...
    integer :: trajectory_npart, trajectory_stride, trajectory_nids
    integer, dimension(100) :: trajectory_ids
    ...
    namelist /run_config/ field_source, file_type, field_file, &
         ...
         write_trajectories, trajectory_npart, trajectory_stride, &
          trajectory_nids, trajectory_ids
    ...
    write_trajectories = cfg%write_trajectories
    trajectory_stride = cfg%trajectory_stride
    trajectory_npart = cfg%trajectory_npart
    trajectory_nids = cfg%trajectory_nids
    trajectory_ids = cfg%trajectory_ids
    ...
    cfg%write_trajectories = write_trajectories
    cfg%trajectory_stride = trajectory_stride
    cfg%trajectory_npart = trajectory_npart
    cfg%trajectory_nids = trajectory_nids
    cfg%trajectory_ids = trajectory_ids
  ...
  subroutine print_config(cfg)
    ...
    write(*,*) " write_trajectories = ", cfg%write_trajectories
    write(*,*) " trajectory_npart = ", cfg%trajectory_npart
    write(*,*) " trajectory_stride = ", cfg%trajectory_stride
    write(*,*) " trajectory_nids = ", cfg%trajectory_nids
